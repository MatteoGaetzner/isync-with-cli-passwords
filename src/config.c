...
char parse_bool(conffile_t *cfile) {
  if (!strcasecmp(cfile->val, "yes") || !strcasecmp(cfile->val, "true") ||
      !strcasecmp(cfile->val, "on") || !strcmp(cfile->val, "1"))
    return 1;
  if (strcasecmp(cfile->val, "no") && strcasecmp(cfile->val, "false") &&
      strcasecmp(cfile->val, "off") && strcmp(cfile->val, "0")) {
    ...
    if (!strcasecmp("Channel", cfile.cmd)) {
      ...
      if (!channel->stores[F]) {
        ...
        if (max_size != UINT_MAX) {
          if (!max_size)
            max_size = UINT_MAX;
          channel->stores[F]->max_size = channel->stores[N]->max_size =
              max_size;
        }
        *channelapp = channel;
