...
/* This function returns the next (cleaned) token in the given line
 * (cfile->rest), which is expected to either be a command or a value */
...
{
    if (!strcasecmp(cfile->val, "yes") || !strcasecmp(cfile->val, "true") || !strcasecmp(cfile->val, "on") || !strcmp(cfile->val, "1"))
        return 1;
    if (strcasecmp(cfile->val, "no") && strcasecmp(cfile->val, "false") && strcasecmp(cfile->val, "off") && strcmp(cfile->val, "0")) {
...
static int getopt_helper(conffile_t* cfile, int* cops, channel_conf_t* conf)
{
    char* arg;
    uint i;
...
int getcline(conffile_t* cfile)
{
    ...
    if (cfile->rest && (arg = get_arg(cfile, ARG_OPTIONAL, NULL))) {
        /* Check whether the previous line that had been
         * processed contained too many tokens */
        error("%s:%d: excess token '%s'\n", cfile->file, cfile->line, arg);
        cfile->err = 1;
    }
    while (fgets(cfile->buf, cfile->bufl, cfile->fp)) {
        /* Iterate over the lines until a line with command and value is found,
         * then save it in cfile->cmd and cfile->val */
        cfile->line++;
...
int load_config(const char* where)
{
    conffile_t cfile;
...
reloop:
    /* In each iteration, one (command,value) pair is extracted
     * and saved in cfile.cmd, cfile.val, until EOF is reached */
    while (getcline(&cfile)) {
        if (!cfile.cmd)
            // Config parsing done
            continue;
        for (i = 0; i < N_DRIVERS; i++)
            // Let each driver try to parse the current store config block
            if (drivers[i]->parse_store(&cfile, &store)) {
                if (store) {
                    ...
                    if (!store->flat_delim)
                        store->flat_delim = "";
                    *storeapp = store;
                    // Prepare next store config struct to get filled
                    storeapp = &store->next;
                    *storeapp = NULL;
                }
                glob_ok = 0;
        ...
        if (!strcasecmp("Channel", cfile.cmd)) {
            ...
            while (getcline(&cfile) && cfile.cmd) {
                ...
                else if (!strcasecmp("Pattern", cfile.cmd) || !strcasecmp("Patterns", cfile.cmd)) {
                    arg = cfile.val;
                    do
            ...
            *chanlistapp = NULL;
            ...
            while (getcline(&cfile) && cfile.cmd) {
                if (!strcasecmp("Channel", cfile.cmd) || !strcasecmp("Channels", cfile.cmd)) {
                    arg = cfile.val;
                    goto addone;
                } else {
            ...
            error("%s:%d: '%s' is not a recognized section-starting or global "
                  "keyword\n",
                cfile.file, cfile.line, cfile.cmd);
            cfile.err = 1;
            while (getcline(&cfile))
