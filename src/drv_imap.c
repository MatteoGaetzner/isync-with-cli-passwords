...
#define IMAP_CMD                                                               \
  ...
  int tag;                                                                     \
                                                                               \
  struct {                                                                     \
    /* Will be called on each continuation request until it resets this        \
     * pointer.                                                                \
     * Needs to invoke bad_callback and return -1 on error, otherwise return   \
     * 0. */                                                                   \
    ...
    char high_prio; /* if command is queued, put it at the front of the queue. \
                     */                                                        \
    char                                                                       \
        wait_check; /* Don't report success until subsequent CHECK success. */ \
    char to_trash;  /* we are storing to trash, not current. */                \
    char                                                                       \
        create; /* create the mailbox if we get an error which suggests so. */ \
...
static imap_cmd_t *new_imap_cmd(uint size) {
...
static void done_imap_cmd(imap_store_t *ctx, imap_cmd_t *cmd, int response) {
...
static void send_imap_cmd(imap_store_t *ctx, imap_cmd_t *cmd) {
  ...
  if (!cmd->param.data) {
    ...
    lbufl =
        nfsnprintf(lenbuf, sizeof(lenbuf), "{%u+}\r\n", cmd->param.data_len);
    litplus = 1;
  }
  if (DFlags & DEBUG_NET) {
...
static int cmd_sendable(imap_store_t *ctx, imap_cmd_t *cmd) {
...
static void flush_imap_cmds(imap_store_t *ctx) {
  imap_cmd_t *cmd;
...
static void finalize_checked_imap_cmds(imap_store_t *ctx, int resp) {
  imap_cmd_t *cmd;
...
static void cancel_pending_imap_cmds(imap_store_t *ctx) {
  imap_cmd_t *cmd;
...
static void cancel_sent_imap_cmds(imap_store_t *ctx) {
  imap_cmd_t *cmd;
...
static void submit_imap_cmd(imap_store_t *ctx, imap_cmd_t *cmd) {
  assert(ctx);
 ...
 * backslash-escaped string literals. Note that this does not automatically add
 * quotes around the printed string, so it is possible to concatenate multiple
 * segments. */
static char *imap_vprintf(const char *fmt, va_list ap) {
  const char *s;
  char *d, *ed;
  char c;
#define MAX_SEGS 16
  ...
  for (;;) {
    ...
    if (!c || c == '%') {
      ...
      if (c == '\\') {
        c = *++fmt;
        if (c != 's') {
          fputs("Fatal: unsupported escaped format specifier. Please report a "
                "bug.\n",
                stderr);
          abort();
        }
        ...
        if (c == '.') {
          c = *++fmt;
          if (c != '*') {
            fputs("Fatal: unsupported string length specification. Please "
                  "report a bug.\n",
                  stderr);
            abort();
          }
...
static void imap_exec(imap_store_t *ctx, imap_cmd_t *cmdp,
                      void (*done)(imap_store_t *ctx, imap_cmd_t *cmd,
                                   int response),
                      const char *fmt, ...) {
  va_list ap;
...
static void transform_box_response(int *response) {
  switch (*response) {
  case RESP_CANCEL:
    *response = DRV_CANCELED;
    break;
  case RESP_NO:
    *response = DRV_BOX_BAD;
    break;
  default:
    *response = DRV_OK;
    break;
...
static void imap_done_simple_box(imap_store_t *ctx ATTR_UNUSED, imap_cmd_t *cmd,
                                 int response) {
...
static void transform_msg_response(int *response) {
  switch (*response) {
  case RESP_CANCEL:
    *response = DRV_CANCELED;
    break;
  case RESP_NO:
    *response = DRV_MSG_BAD;
    break;
  default:
    *response = DRV_OK;
    break;
...
static void imap_done_simple_msg(imap_store_t *ctx ATTR_UNUSED, imap_cmd_t *cmd,
                                 int response) {
  imap_cmd_simple_t *cmdp = (imap_cmd_simple_t *)cmd;
...
static imap_cmd_refcounted_state_t *imap_refcounted_new_state(uint sz) {
...
static imap_cmd_t *imap_refcounted_new_cmd(imap_cmd_refcounted_state_t *sts) {
  imap_cmd_refcounted_t *cmd =
      (imap_cmd_refcounted_t *)new_imap_cmd(sizeof(*cmd));
  cmd->state = sts;
  sts->ref_count++;
  return &cmd->gen;
}
...
#define DONE_REFCOUNTED_STATE_ARGS(sts, finalize, ...)                         \
  if (!--sts->ref_count) {                                                     \
    finalize sts->callback(sts->ret_val, __VA_ARGS__, sts->callback_aux);      \
    free(sts);                                                                 \
  }

static void transform_refcounted_box_response(imap_cmd_refcounted_state_t *sts,
                                              int response) {
  switch (response) {
  case RESP_CANCEL:
...
static void transform_refcounted_msg_response(imap_cmd_refcounted_state_t *sts,
                                              int response) {
  switch (response) {
  case RESP_CANCEL:
    sts->ret_val = DRV_CANCELED;
...
static const char *imap_strchr(const char *s, char tc) {
  for (;; s++) {
    char c = *s;
    if (c == '\\')
      c = *++s;
...
static int is_opt_atom(list_t *list) {
...
static int is_list(list_t *list) { return list && list->val == LIST; }
...
enum { LIST_OK, LIST_PARTIAL, LIST_BAD };

static int parse_imap_list(imap_store_t *ctx, char **sp,
...
static void parse_list_init(parse_list_state_t *sts) {
  sts->need_bytes = -1;
  sts->level = 1;
  sts->head = NULL;
...
static int parse_list_continue(imap_store_t *ctx, char *s) {
  list_t *list;
  int resp;
...
static int parse_list(imap_store_t *ctx, char *s,
                      int (*cb)(imap_store_t *ctx, list_t *list, char *s)) {
...
static int parse_namespace_rsp(imap_store_t *ctx, list_t *list, char *s) {
...
static int parse_namespace_rsp_p2(imap_store_t *ctx, list_t *list ATTR_UNUSED,
                                  char *s) {
  return parse_list(ctx, s, parse_namespace_rsp_p3);
}

static int parse_namespace_rsp_p3(imap_store_t *ctx ATTR_UNUSED,
                                  list_t *list ATTR_UNUSED,
                                  char *s ATTR_UNUSED) {
  return LIST_OK;
}
...
static int parse_fetched_flags(list_t *list, uchar *flags, uchar *status) {
...
static void parse_fetched_header(char *val, uint uid, char **tuid, char **msgid,
                                 uint *msgid_len) {
  char *end;
  int off, in_msgid = 0;
  for (; (end = strchr(val, '\n')); val = end + 1) {
...
static int parse_fetch_rsp(imap_store_t *ctx, list_t *list,
                           char *s ATTR_UNUSED) {
...
static void parse_capability(imap_store_t *ctx, char *cmd) {
  char *arg;
  uint i;
...
static int parse_response_code(imap_store_t *ctx, imap_cmd_t *cmd, char *s) {
  char *arg, *earg, *p;
...
static int parse_list_rsp(imap_store_t *ctx, list_t *list, char *cmd) {
  list_t *lp;
...
static int parse_list_rsp_p1(imap_store_t *ctx, list_t *list,
...
static int is_inbox(imap_store_t *ctx, const char *arg, int argl) {
...
static void normalize_INBOX(imap_store_t *ctx, char *arg, int argl) {
  if (is_inbox(ctx, arg, argl))
    memcpy(arg, "INBOX", 5);
}

static int parse_list_rsp_p2(imap_store_t *ctx, list_t *list,
                             char *cmd ATTR_UNUSED) {
  string_list_t *narg;
  char *arg, c;
  int argl;
  ...
  if (argl > 1000) {
    warn(
        "IMAP warning: ignoring unreasonably long mailbox name '%.100s[...]'\n",
        arg);
    return LIST_OK;
  }
  ...
  if (argl >= 5 &&
      !memcmp(arg + argl - 5, ".lock", 5)) /* workaround broken servers */
    return LIST_OK;
  ...
  for (const char *p = narg->string, *sp = p;;) {
    if (!(c = *p) || c == '/') {
      ...
      if (pcl == 2 && sp[0] == '.' && sp[1] == '.') {
        error("IMAP error: LIST'd mailbox name '%s' contains '..' component - "
              "THIS MIGHT BE AN ATTEMPT TO HACK YOU!\n",
              narg->string);
        free(narg);
        return LIST_BAD;
...
static int prepare_name(char **buf, const imap_store_t *ctx, const char *prefix,
                        const char *name) {
...
static int prepare_box(char **buf, const imap_store_t *ctx) {
...
static int prepare_trash(char **buf, const imap_store_t *ctx) {
...
static void imap_socket_read(void *aux) {
  ...
  for (;;) {
    ...
    if (cmd == (void *)~0) {
      if (!ctx->expectEOF)
        error("IMAP error: unexpected EOF from %s\n", ctx->conn.name);
      /* A clean shutdown sequence ends with bad_callback as well (see
       * imap_cleanup()). */
      break;
    }
    if (DFlags & DEBUG_NET) {
      ...
      if (!strcmp("OK", arg)) {
        ...
        if (!strcmp("NO", arg)) {
          if (cmdp->param.create && cmd &&
              starts_with(cmd, -1, "[TRYCREATE]",
                          11)) { /* APPEND or UID COPY */
            imap_cmd_trycreate_t *cmd2 =
                (imap_cmd_trycreate_t *)new_imap_cmd(sizeof(*cmd2));
            cmd2->orig_cmd = cmdp;
            cmd2->param.high_prio = 1;
            p = strchr(cmdp->cmd, '"');
            imap_exec(ctx, &cmd2->gen, get_cmd_result_p2, "CREATE %.*s",
        ...
        error("IMAP command '%s' returned an error: %s %s\n",
              starts_with(cmdp->cmd, -1, "LOGIN", 5) ? "LOGIN <user> <pass>"
...
static void get_cmd_result_p2(imap_store_t *ctx, imap_cmd_t *cmd,
                              int response) {
...
static void imap_cancel_store(store_t *gctx) {
...
static void imap_set_bad_callback(store_t *gctx, void (*cb)(void *aux),
                                  void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_cancel_unowned(void *gctx) {
...
static void imap_free_store(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_cleanup_p2(imap_store_t *ctx, imap_cmd_t *cmd ATTR_UNUSED,
                            int response) {
  if (response == RESP_NO)
    imap_cancel_store(&ctx->gen);
...
static store_t *imap_alloc_store(store_conf_t *conf, const char *label) {
  ...
  socket_init(&ctx->conn, &srvc->sconf,
              (void (*)(void *))imap_invoke_bad_callback, imap_socket_read,
              (void (*)(void *))flush_imap_cmds, ctx);
...
static void imap_connect_store(store_t *gctx, void (*cb)(int sts, void *aux),
                               void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_open_store_connected(int ok, void *aux) {
  imap_store_t *ctx = (imap_store_t *)aux;
...
#ifdef HAVE_LIBSSL
static void imap_open_store_tlsstarted1(int ok, void *aux) {
  imap_store_t *ctx = (imap_store_t *)aux;
...
static void imap_open_store_greeted(imap_store_t *ctx) {
  socket_expect_activity(&ctx->conn, 0);
  if (!ctx->caps)
...
static void imap_open_store_p2(imap_store_t *ctx, imap_cmd_t *cmd ATTR_UNUSED,
                               int response) {
  if (response == RESP_NO)
...
static void imap_open_store_authenticate(imap_store_t *ctx) {
#ifdef HAVE_LIBSSL
...
#ifdef HAVE_LIBSSL
static void imap_open_store_authenticate_p2(imap_store_t *ctx,
...
static void imap_open_store_tlsstarted2(int ok, void *aux) {
  imap_store_t *ctx = (imap_store_t *)aux;
...
static void imap_open_store_authenticate_p3(imap_store_t *ctx,
                                            imap_cmd_t *cmd ATTR_UNUSED,
                                            int response) {
  if (response == RESP_NO)
    imap_open_store_bail(ctx, FAIL_FINAL);
...
static char *cred_from_cmd(const char *cred, const char *cmd,
                           const char *srv_name) {
  FILE *fp;
  int ret;
...
static const char *ensure_user(imap_server_conf_t *srvc) {
  if (!srvc->user) {
    if (srvc->user_cmd) {
...
static const char *ensure_password(imap_server_conf_t *srvc) {
  if (!srvc->pass) {
    if (srvc->pass_cmd) {
      ...
      OSStatus ret = SecKeychainFindInternetPassword(
          ...
          strlen(srvc->sconf.host), srvc->sconf.host, 0, NULL, // securityDomain
          strlen(srvc->user), srvc->user, 0, NULL,             // path
          0, // port - we could use it, but it seems pointless
          kSecProtocolTypeIMAP, kSecAuthenticationTypeDefault, &password_length,
          &password_data,
          NULL); // itemRef
      ...
      /* getpass() returns a pointer to a static buffer. Make a copy for long
       * term storage. */
...
static sasl_callback_t sasl_callbacks[] = {{SASL_CB_USER, NULL, NULL},
                                           {SASL_CB_AUTHNAME, NULL, NULL},
                                           {SASL_CB_PASS, NULL, NULL},
                                           {SASL_CB_LIST_END, NULL, NULL}};

static int process_sasl_interact(sasl_interact_t *interact,
...
static int process_sasl_step(imap_store_t *ctx, int rc, const char *in,
                             uint in_len, sasl_interact_t *interact,
                             const char **out, uint *out_len) {
  imap_server_conf_t *srvc = ctx->conf->server;
...
static int decode_sasl_data(const char *prompt, char **in, uint *in_len) {
  if (prompt) {
    int rc;
...
static int encode_sasl_data(const char *out, uint out_len, char **enc,
                            uint *enc_len) {
  int rc;
...
static int do_sasl_auth(imap_store_t *ctx, imap_cmd_t *cmdp ATTR_UNUSED,
                        const char *prompt) {
  int rc, ret, iovcnt = 0;
...
static void done_sasl_auth(imap_store_t *ctx, imap_cmd_t *cmd ATTR_UNUSED,
                           int response) {
  if (response == RESP_OK && ctx->sasl_cont) {
    ...
    if (process_sasl_step(ctx, rc, NULL, 0, interact, &out, &out_len) < 0)
      warn("Warning: SASL reported failure despite successful IMAP "
           "authentication. Ignoring...\n");
    else if (out_len > 0)
      warn("Warning: SASL wants more steps despite successful IMAP "
           "authentication. Ignoring...\n");
  }
...
static void imap_open_store_authenticate2(imap_store_t *ctx) {
  imap_server_conf_t *srvc = ctx->conf->server;
  ...
  // Ensure that there are no leftovers from previous runs. This is needed in
  // case the credentials have a timing dependency or otherwise lose validity
  // after use.
  if (srvc->user_cmd) {
    free(srvc->user);
    srvc->user = NULL;
  ...
  if (saslend != saslmechs) {
    ...
    rc = sasl_client_new("imap", srvc->sconf.host, NULL, NULL, NULL, 0,
                         &ctx->sasl);
    if (rc != SASL_OK) {
      if (rc == SASL_NOMECH)
        goto notsasl;
    ...
    if (process_sasl_step(ctx, rc, NULL, 0, interact, CAP(SASLIR) ? &out : NULL,
                          &out_len) < 0)
      goto bail;
    if (out) {
      if (!out_len)
    ...
    imap_exec(ctx, cmd, done_sasl_auth,
              enc ? "AUTHENTICATE %s %s" : "AUTHENTICATE %s", gotmech, enc);
    free(enc);
    return;
  notsasl:
    if (!ctx->sasl || sasl_listmech(ctx->sasl, NULL, "", " ", "", &saslavail,
                                    NULL, NULL) != SASL_OK)
      saslavail = "(none)";
    if (!auth_login) {
...
static void imap_open_store_authenticate2_p2(imap_store_t *ctx,
                                             imap_cmd_t *cmd ATTR_UNUSED,
                                             int response) {
  if (response == RESP_NO)
    imap_open_store_bail(ctx, FAIL_FINAL);
...
static void imap_open_store_compress(imap_store_t *ctx) {
#ifdef HAVE_LIBZ
...
#ifdef HAVE_LIBZ
static void imap_open_store_compress_p2(imap_store_t *ctx,
                                        imap_cmd_t *cmd ATTR_UNUSED,
                                        int response) {
  if (response == RESP_NO) {
...
static void imap_open_store_namespace(imap_store_t *ctx) {
...
static void imap_open_store_namespace_p2(imap_store_t *ctx,
                                         imap_cmd_t *cmd ATTR_UNUSED,
                                         int response) {
  if (response == RESP_NO) {
    imap_open_store_bail(ctx, FAIL_FINAL);
...
static void imap_open_store_namespace2(imap_store_t *ctx) {
...
static void imap_open_store_finalize(imap_store_t *ctx) {
  ctx->state = SST_GOOD;
  if (!ctx->prefix)
    ctx->prefix = "";
  else
...
#ifdef HAVE_LIBSSL
static void imap_open_store_ssl_bail(imap_store_t *ctx) {
...
static void imap_open_store_bail(imap_store_t *ctx, int failed) {
...
static int imap_select_box(store_t *gctx, const char *name) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static const char *imap_get_box_path(store_t *gctx ATTR_UNUSED) { return NULL; }
...
static void imap_open_box(store_t *gctx,
  ...
  cmd->param.failok = 1;
  imap_exec(ctx, &cmd->gen, imap_open_box_p2, "SELECT \"%\\s\"", buf);
  free(buf);
}

static void imap_open_box_p2(imap_store_t *ctx, imap_cmd_t *gcmd,
                             int response) {
  ...
  imap_exec(ctx, &cmd->gen, imap_open_box_p3, "UID FETCH * (UID)");
}

static void imap_open_box_p3(imap_store_t *ctx, imap_cmd_t *gcmd,
                             int response) {
  imap_cmd_open_box_t *cmdp = (imap_cmd_open_box_t *)gcmd;
...
static void imap_open_box_p4(imap_store_t *ctx, imap_cmd_open_box_t *cmdp,
                             int response) {
  transform_box_response(&response);
  cmdp->callback(response, ctx->uidvalidity, cmdp->callback_aux);
}

static uint imap_get_uidnext(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;

  return ctx->uidnext;
}

static xint imap_get_supported_flags(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_create_box(store_t *gctx, void (*cb)(int sts, void *aux),
                            void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
  ...
  imap_exec(ctx, &cmd->gen, imap_done_simple_box, "CREATE \"%\\s\"", buf);
  free(buf);
}

/******************* imap_delete_box *******************/

static int imap_confirm_box_empty(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_delete_box(store_t *gctx, void (*cb)(int sts, void *aux),
                            void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
  imap_cmd_simple_t *cmd;
...
static void imap_delete_box_p2(imap_store_t *ctx, imap_cmd_t *gcmd,
                               int response) {
  ...
  imap_exec(ctx, &cmd->gen, imap_done_simple_box, "DELETE \"%\\s\"", buf);
  free(buf);
}

static int imap_finish_delete_box(store_t *gctx ATTR_UNUSED) { return DRV_OK; }

/******************* imap_load_box *******************/

static uint imap_prepare_load_box(store_t *gctx, uint opts) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_set_range(imap_range_t *ranges, uint *nranges, int low_flags,
                           int high_flags, uint maxlow) {
  if (low_flags != high_flags) {
    for (uint r = 0; r < *nranges; r++) {
      ...
      if (ranges[r].last != maxlow) {
        ...
        memmove(&ranges[r + 1], &ranges[r],
                ((*nranges)++ - r) * sizeof(*ranges));
...
typedef union {
  ...
  struct {
    IMAP_CMD_REFCOUNTED_STATE
    void (*callback)(int sts, message_t *msgs, int total_msgs, int recent_msgs,
                     void *aux);
    void *callback_aux;
  };
...
static void imap_load_box(store_t *gctx, uint minuid, uint maxuid, uint finduid,
  ...
  if (!ctx->total_msgs) {
    ...
    for (uint i = 0; i < excs.size;) {
      ...
      imap_submit_load(ctx, buf,
                       shifted_bit(ctx->opts, OPEN_OLD_IDS, WantMsgids), sts);
    }
    if (maxuid == UINT_MAX)
...
static int imap_sort_msgs_comp(const void *a_, const void *b_) {
...
static void imap_sort_msgs(imap_store_t *ctx) {
...
static void imap_submit_load(imap_store_t *ctx, const char *buf, int flags,
...
static void imap_submit_load_p2(imap_store_t *ctx, imap_cmd_t *cmd,
                                int response) {
  imap_load_box_state_t *sts =
      (imap_load_box_state_t *)((imap_cmd_refcounted_t *)cmd)->state;

  transform_refcounted_box_response(&sts->gen, response);
  imap_submit_load_p3(ctx, sts);
}

static void imap_submit_load_p3(imap_store_t *ctx, imap_load_box_state_t *sts) {
  DONE_REFCOUNTED_STATE_ARGS(
      sts,
      {
        ctx->fetch_sts = FetchNone;
...
static void imap_fetch_msg(store_t *ctx, message_t *msg, msg_data_t *data,
                           int minimal, void (*cb)(int sts, void *aux),
                           void *aux) {
  ...
  imap_exec((imap_store_t *)ctx, &cmd->gen.gen, imap_fetch_msg_p2,
            ...
            (data->date == -1) ? "INTERNALDATE " : "", minimal ? "HEADER" : "");
}

static void imap_fetch_msg_p2(imap_store_t *ctx, imap_cmd_t *gcmd,
                              int response) {
...
static uint imap_make_flags(int flags, char *buf) {
  const char *s;
  uint i, d;
...
static void imap_flags_helper(imap_store_t *ctx, uint uid, char what, int flags,
  ...
  imap_exec(ctx, cmd, imap_set_flags_p2, "UID STORE %u %cFLAGS.SILENT %s", uid,
            what, buf);
}

static void imap_set_msg_flags(store_t *gctx, message_t *msg, uint uid, int add,
                               int del, void (*cb)(int sts, void *aux),
                               void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_set_flags_p2(imap_store_t *ctx ATTR_UNUSED, imap_cmd_t *cmd,
                              int response) {
  imap_set_msg_flags_state_t *sts =
      (imap_set_msg_flags_state_t *)((imap_cmd_refcounted_t *)cmd)->state;

  transform_refcounted_msg_response(&sts->gen, response);
  imap_set_flags_p3(sts);
}

static void imap_set_flags_p3(imap_set_msg_flags_state_t *sts) {
...
static void imap_close_box(store_t *gctx, void (*cb)(int sts, void *aux),
                           void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_close_box_p2(imap_store_t *ctx ATTR_UNUSED, imap_cmd_t *cmd,
                              int response) {
  imap_expunge_state_t *sts =
      (imap_expunge_state_t *)((imap_cmd_refcounted_t *)cmd)->state;

  transform_refcounted_box_response(&sts->gen, response);
  imap_close_box_p3(sts);
}

static void imap_close_box_p3(imap_expunge_state_t *sts) {
  DONE_REFCOUNTED_STATE(sts)
}

/******************* imap_trash_msg *******************/

static void imap_trash_msg(store_t *gctx, message_t *msg,
  ...
  imap_exec(ctx, &cmd->gen, imap_done_simple_msg,
            CAP(MOVE) ? "UID MOVE %u \"%\\s\"" : "UID COPY %u \"%\\s\"",
            msg->uid, buf);
  free(buf);
}
...
static void imap_store_msg(store_t *gctx, msg_data_t *data, int to_trash,
                           void (*cb)(int sts, uint uid, void *aux),
                           void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
  ...
  if (data->date) {
    ...
    strftime(datestr, sizeof(datestr), "%d-%b-%Y %H:%M:%S %z",
             localtime(&data->date));
    DIAG_POP
    imap_exec(ctx, &cmd->gen, imap_store_msg_p2, "APPEND \"%\\s\" %s\"%\\s\" ",
              buf, flagstr, datestr);
  } else {
    imap_exec(ctx, &cmd->gen, imap_store_msg_p2, "APPEND \"%\\s\" %s", buf,
              flagstr);
  }
  free(buf);
}

static void imap_store_msg_p2(imap_store_t *ctx ATTR_UNUSED, imap_cmd_t *cmd,
                              int response) {
...
static void imap_find_new_msgs(store_t *gctx, uint newuid,
  ...
  cmd->uid = newuid;
  // Some servers fail to enumerate recently STOREd messages without syncing
  // first.
  imap_exec((imap_store_t *)ctx, &cmd->gen, imap_find_new_msgs_p2, "CHECK");
}

static void imap_find_new_msgs_p2(imap_store_t *ctx, imap_cmd_t *gcmd,
                                  int response) {
  ...
  cmd->uid = cmdp->uid;
  imap_exec(ctx, &cmd->gen, imap_find_new_msgs_p3, "UID FETCH * (UID)");
}

static void imap_find_new_msgs_p3(imap_store_t *ctx, imap_cmd_t *gcmd,
                                  int response) {
  imap_cmd_find_new_t *cmdp = (imap_cmd_find_new_t *)gcmd;
  imap_cmd_find_new_t *cmd;
...
static void imap_find_new_msgs_p4(imap_store_t *ctx ATTR_UNUSED,
                                  imap_cmd_t *gcmd, int response) {
  imap_cmd_find_new_t *cmdp = (imap_cmd_find_new_t *)gcmd;
...
static void imap_list_store(store_t *gctx, int flags,
                            void (*cb)(int sts, string_list_t *boxes,
                                       void *aux),
                            void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
  imap_store_conf_t *cfg = ctx->conf;
  ...
  // 'listed' already containing a particular value effectively removes it from
  // 'flags'. This matrix determines what to query, and what comes out as a side
  // effect. The lowercase letters indicate unnecessary results; the queries are
  // done this way to have non-overlapping result sets, so subsequent calls
...
static void imap_list_store_p2(imap_store_t *ctx, imap_cmd_t *cmd,
                               int response) {
  imap_list_store_state_t *sts =
      (imap_list_store_state_t *)((imap_cmd_refcounted_t *)cmd)->state;

  transform_refcounted_box_response(&sts->gen, response);
  imap_list_store_p3(ctx, sts);
}

static void imap_list_store_p3(imap_store_t *ctx,
                               imap_list_store_state_t *sts) {
...
static void imap_cancel_cmds(store_t *gctx, void (*cb)(void *aux), void *aux) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_commit_cmds(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static void imap_commit_cmds_p2(imap_store_t *ctx, imap_cmd_t *cmd ATTR_UNUSED,
                                int response) {
...
static uint imap_get_memory_usage(store_t *gctx) {
  imap_store_t *ctx = (imap_store_t *)gctx;
...
static int imap_get_fail_state(store_conf_t *gconf) {
...
static int imap_parse_store(conffile_t *cfg, store_conf_t **storep) {
  ...
  /* If this function is responsible for the given settings block,
   * then it processes it, otherwise it returns 0 */
  ...
  // Parse exactly one IMAP settings block
  while (getcline(cfg) && cfg->cmd) {
    if (!strcasecmp("Host", cfg->cmd)) {
      ...
      if (starts_with(arg, -1, "//", 2))
        arg += 2;
      if (arg != cfg->val)
        warn("%s:%d: Notice: URL notation is deprecated; use a plain host name "
             "and possibly 'SSLType IMAPS' instead\n",
             cfg->file, cfg->line);
      ...
      if (access(server->sconf.cert_file, R_OK)) {
        sys_error("%s:%d: CertificateFile '%s'", cfg->file, cfg->line,
      ...
      if (access(server->sconf.client_certfile, R_OK)) {
        sys_error("%s:%d: ClientCertificate '%s'", cfg->file, cfg->line,
      ...
      if (access(server->sconf.client_keyfile, R_OK)) {
        sys_error("%s:%d: ClientKey '%s'", cfg->file, cfg->line,
  ...
  if (!store || !store->server) {
    ...
    if ((use_tlsv1 & use_tlsv11 & use_tlsv12 & use_tlsv13) != -1 ||
      ...
      if (server->ssl_type >= 0 || server->sconf.ssl_versions >= 0) {
        error("%s '%s': The deprecated UseSSL*, UseTLS*, UseIMAPS, and "
              "RequireSSL options are mutually exclusive with SSLType and "
              "SSLVersions.\n",
              type, name);
        cfg->err = 1;
        return 1;
      }
      warn("Notice: %s '%s': UseSSL*, UseTLS*, UseIMAPS, and RequireSSL are "
           "deprecated. Use SSLType and SSLVersions instead.\n",
           type, name);
      server->sconf.ssl_versions =
          (use_tlsv1 == 0 ? 0 : TLSv1) | (use_tlsv11 != 1 ? 0 : TLSv1_1) |
          (use_tlsv12 != 1 ? 0 : TLSv1_2) | (use_tlsv13 != 1 ? 0 : TLSv1_3);
      if (use_imaps == 1) {
        server->ssl_type = SSL_IMAPS;
      } else if (require_ssl) {
        server->ssl_type = SSL_STARTTLS;
    ...
    if (require_cram >= 0) {
      if (server->auth_mechs) {
        error("%s '%s': The deprecated RequireCRAM option is mutually "
              "exclusive with AuthMech.\n",
              type, name);
        cfg->err = 1;
        return 1;
      }
      warn(
          "Notice: %s '%s': RequireCRAM is deprecated. Use AuthMech instead.\n",
          type, name);
      if (require_cram)
...
static uint imap_get_caps(store_t *gctx ATTR_UNUSED) {
  return DRV_CRLF | DRV_VERBOSE | DRV_ASYNC;
}

struct driver imap_driver = {
    imap_get_caps,          imap_parse_store,      imap_cleanup,
    imap_alloc_store,       imap_set_bad_callback, imap_connect_store,
    imap_free_store,        imap_cancel_store,     imap_list_store,
    imap_select_box,        imap_get_box_path,     imap_create_box,
    imap_open_box,          imap_get_uidnext,      imap_get_supported_flags,
    imap_confirm_box_empty, imap_delete_box,       imap_finish_delete_box,
    imap_prepare_load_box,  imap_load_box,         imap_fetch_msg,
    imap_store_msg,         imap_find_new_msgs,    imap_set_msg_flags,
    imap_trash_msg,         imap_close_box,        imap_cancel_cmds,
    imap_commit_cmds,       imap_get_memory_usage, imap_get_fail_state,
};
