...
static INLINE void imap_ref(imap_store_t* ctx)
{
    ++ctx->ref_count;
}
...
static int cmd_sendable(imap_store_t* ctx, imap_cmd_t* cmd)
{
    ...
    if (ctx->in_progress) {
        ...
        imap_cmd_t* cmdp = (imap_cmd_t*)((char*)ctx->in_progress_append - offsetof(imap_cmd_t, next));
...
static int parse_response_code(imap_store_t* ctx, imap_cmd_t* cmd, char* s)
{
    ...
    if (!strcmp("UIDVALIDITY", arg)) {
        if (!(arg = next_arg(&s)) || (ctx->uidvalidity = strtoul(arg, &earg, 10), *earg != ']')) {
            error("IMAP error: malformed UIDVALIDITY status\n");
            return RESP_CANCEL;
        }
    } else if (!strcmp("UIDNEXT", arg)) {
        if (!(arg = next_arg(&s)) || (ctx->uidnext = strtoul(arg, &earg, 10), *earg != ']')) {
        ...
        *p = 0;
        ...
        if (!(arg = next_arg(&s)) || (ctx->uidvalidity = strtoul(arg, &earg, 10), *earg) || !(arg = next_arg(&s)) || (cmd->param.uid = strtoul(arg, &earg, 10), *earg != ']')) {
...
static void imap_socket_read(void* aux)
{
    ...
    for (;;) {
        ...
        if (*arg == '*') {
            ...
            cmdp = (imap_cmd_t*)((char*)ctx->in_progress_append - offsetof(imap_cmd_t, next));
            if (cmdp->param.data) {
            ...
            if (!strcmp("OK", arg)) {
                ...
                if (!strcmp("NO", arg)) {
                    if (cmdp->param.create && cmd && starts_with(cmd, -1, "[TRYCREATE]", 11)) { /* APPEND or UID COPY */
                        imap_cmd_trycreate_t* cmd2 = (imap_cmd_trycreate_t*)new_imap_cmd(sizeof(*cmd2));
                        cmd2->orig_cmd = cmdp;
...
static int imap_parse_store(conffile_t* cfg, store_conf_t** storep)
{
    ...
    while (getcline(cfg) && cfg->cmd) {
        ...
        } else if (!strcasecmp("DisableExtension", cfg->cmd) || !strcasecmp("DisableExtensions", cfg->cmd)) {
            arg = cfg->val;
            do {
        ...
        } else if (!strcasecmp("SSLVersion", cfg->cmd) || !strcasecmp("SSLVersions", cfg->cmd)) {
...
#endif
        else if (!strcasecmp("AuthMech", cfg->cmd) || !strcasecmp("AuthMechs", cfg->cmd)) {
            arg = cfg->val;
            do
    ...
    if (!store || !store->server) {
        ...
        if ((use_tlsv1 & use_tlsv11 & use_tlsv12 & use_tlsv13) != -1 || use_imaps >= 0 || require_ssl >= 0) {
            ...
            warn("Notice: %s '%s': UseSSL*, UseTLS*, UseIMAPS, and RequireSSL are "
                ...
                type, name);
            server->sconf.ssl_versions = (use_tlsv1 == 0 ? 0 : TLSv1) | (use_tlsv11 != 1 ? 0 : TLSv1_1) | (use_tlsv12 != 1 ? 0 : TLSv1_2) | (use_tlsv13 != 1 ? 0 : TLSv1_3);
            if (use_imaps == 1) {
                server->ssl_type = SSL_IMAPS;
            } else if (require_ssl) {
                server->ssl_type = SSL_STARTTLS;
