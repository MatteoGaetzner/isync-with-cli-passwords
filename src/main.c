...
static void ATTR_NORETURN version(void) {
...
#ifdef __linux__
static void ATTR_NORETURN crashHandler(int n) {
  int dpid;
...
void stats(void) {
  ...
  if (cols < 0 && (!(cs = getenv("COLUMNS")) || !(cols = atoi(cs))))
    cols = 80;
  ll = sprintf(buf[2], "C: %d/%d  B: %d/%d", chans_done, chans_total,
               boxes_done, boxes_total);
  cls = (cols - ll - 10) / 2;
  for (t = 0; t < 2; t++) {
    l = sprintf(buf[t], "+%d/%d *%d/%d #%d/%d", new_done[t], new_total[t],
                flags_done[t], flags_total[t], trash_done[t], trash_total[t]);
    if (l > cls)
      buf[t][cls - 1] = '~';
  }
...
static int matches(const char *t, const char *p) {
  for (;;) {
    if (!*p)
      return !*t;
    if (*p == '*') {
...
static int is_inbox(const char *name) {
  return starts_with(name, -1, "INBOX", 5) && (!name[5] || name[5] == '/');
}

static int cmp_box_names(const void *a, const void *b) {
...
static char **filter_boxes(string_list_t *boxes, const char *prefix,
...
static void merge_actions(channel_conf_t *chan, int ops[], int have, int mask,
                          int def) {
  if (ops[F] & have) {
    chan->ops[F] &= ~mask;
...
static chan_ent_t *add_channel(chan_ent_t ***chanapp, channel_conf_t *chan,
                               int ops[]) {
...
static chan_ent_t *add_named_channel(chan_ent_t ***chanapp, char *channame,
                                     int ops[]) {
  channel_conf_t *chan;
  chan_ent_t *ce;
...
#define MVARS(aux)                                                             \
  int t = *(int *)aux;                                                         \
  main_vars_t *mvars = (main_vars_t *)(((char *)(&((int *)aux)[-t])) -         \
                                       offsetof(main_vars_t, t));
    ...
    if (!ochar || !*ochar) {
      ...
      if (argv[oind][1] == '-') {
        ...
        if (!strcmp(opt, "config")) {
          ...
          config = opt + 7;
        else if (!strcmp(opt, "passwords")) {
          n_stdin_creds = read_credentials(creds);
        ...
        rlcac:
          ...
          case XOP_PULL:
            ops[N] |= op & OP_MASK_TYPE;
            break;
          case XOP_PUSH:
            ops[F] |= op & OP_MASK_TYPE;
            break;
          default:
            cops |= op;
            break;
...
static void cancel_prep_done(void *aux) {
  MVARS(aux)
...
static void sync_chans(main_vars_t *mvars, int ent) {
  ...
  do {
    ...
    for (t = 0; t < 2; t++) {
      int st = mvars->chan->stores[t]->driver->get_fail_state(
          mvars->chan->stores[t]);
      if (st != FAIL_TEMP) {
        info("Skipping due to %sfailed %s store %s.\n",
             (st == FAIL_WAIT) ? "temporarily " : "", str_fn[t],
    ...
    for (t = 0; t < 2; t++) {
      store_t *ctx =
          mvars->drv[t]->alloc_store(mvars->chan->stores[t], labels[t]);
      if ((DFlags & DEBUG_DRV) ||
          ((DFlags & FORCEASYNC) && !(dcaps[t] & DRV_ASYNC))) {
  ...
  opened:
    ...
    if (!mvars->chanptr->boxlist && mvars->chan->patterns) {
      ...
      boxes[F] = filter_boxes(mvars->boxes[F], mvars->chan->boxes[F],
                              mvars->chan->patterns);
      boxes[N] = filter_boxes(mvars->boxes[N], mvars->chan->boxes[N],
                              mvars->chan->patterns);
      ...
      for (mb = sb = 0;;) {
        ...
        if (!(cmp = !mname - !sname) &&
          ...
          mbox->present[N] =
              (!mb && !strcmp(mbox->name, "INBOX")) ? BOX_PRESENT : BOX_ABSENT;
          mb++;
        } else {
          mbox->name = sname;
          mbox->present[F] =
              (!sb && !strcmp(mbox->name, "INBOX")) ? BOX_PRESENT : BOX_ABSENT;
  ...
  syncml:
    ...
    if (mvars->chanptr->boxlist) {
      ...
      if (!mvars->list) {
        ...
        sync_boxes(mvars->ctx, mvars->chan->boxes, present, mvars->chan,
                   done_sync, mvars);
        mvars->skip = 1;
      syncw:
        mvars->cben = 1;
      ...
      syncone:
        ...
        printf("%s <=> %s\n", nz(mvars->chan->boxes[F], "INBOX"),
               nz(mvars->chan->boxes[N], "INBOX"));
    }
        ...
        if (*pat != '!') {
          ...
          if (starts_with(buf, bufl, "INBOX", 5)) {
            ...
            if (!c) {
              ...
              /* It can be both INBOX and Path, but don't require Path to be
               * configured. */
    ...
    for (box = boxes; box; box = box->next) {
      if (mvars->ctx[t]->conf->flat_delim[0]) {
        ...
        if (map_name(box->string, (char **)&nbox,
                     offsetof(string_list_t, string),
                     mvars->ctx[t]->conf->flat_delim, "/") < 0) {
          error("Error: flattened mailbox name '%s' contains canonical "
                "hierarchy delimiter\n",
                box->string);
          mvars->ret = mvars->skip = 1;
        } else {
...
static int sync_listed_boxes(main_vars_t *mvars, box_ent_t *mbox) {
  if (mvars->chan->boxes[F] || mvars->chan->boxes[N]) {
    ...
    if (!mvars->list) {
      ...
      sync_boxes(mvars->ctx, (const char *const *)mvars->names, mbox->present,
