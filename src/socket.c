...
verify_hostname(X509* cert, const char* hostname)
{
    ...
    if ((subj_alt_names = X509_get_ext_d2i(cert, NID_subject_alt_name, NULL, NULL))) {
        ...
        for (i = 0; i < num_subj_alt_names; i++) {
            ...
            if (subj_alt_name->type == GEN_DNS && strlen((const char*)subj_alt_name->d.ia5->data) == (size_t)subj_alt_name->d.ia5->length && host_matches(hostname, (const char*)(subj_alt_name->d.ia5->data))) {
                found = 1;
                break;
            }
        }
...
void socket_connect(conn_t* sock, void (*cb)(int ok, void* aux))
{
...
char* socket_read_line(conn_t* b)
{
    char *p, *s;
    uint n;
...
void socket_write(conn_t* conn, conn_iovec_t* iov, int iovcnt)
{
    int i;
