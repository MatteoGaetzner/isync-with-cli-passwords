...
void vdebug(int cat, const char* msg, va_list va)
{
    if (DFlags & cat) {
        vprintf(msg, va);
        fflush(stdout);
...
void vdebugn(int cat, const char* msg, va_list va)
{
    if (DFlags & cat) {
        vprintf(msg, va);
        fflush(stdout);
...
void vsys_error(const char* msg, va_list va)
{
    char buf[1024];
...
int find_uint_array(uint_array_t array, uint value)
{
...
void conf_wakeup(wakeup_t* tmr, int to)
{
